...
"1"="$ExtensionId;$updateXmlUrl"
"@
    Set-Content -Path (Join-Path $projectRoot "gsecurity.reg") -Value $regContent -Encoding ascii
}
else {
    Write-Warning "ExtensionId is empty. Keeping existing update.xml and gsecurity.reg unchanged."
}
...
Write-Host " - gsecurity.reg: $(Join-Path $projectRoot "gsecurity.reg")"
